...
import { BackendError, RemoteBackend, isInside } from "./backend.js";
/** One text content block (the only render shape these tools emit). */
function text(value) {
    return [{ type: 'text', text: value }];
}
/** Build every remote_* tool (registered by the host half). */
export function makeWorkspaceTools(deps) {
    ...
    return [
        ...
        defineTool({
            ...
            output: {
                ...
                render: (_args, value) => {
                    if (value.ok !== true)
                        return text(`remote_ls failed: ${value.error ?? 'unknown error'}`);
        ...
        defineTool({
            ...
            output: {
                ...
                render: (_args, value) => {
                    if (value.ok !== true)
                        return text(`remote_read failed: ${value.error ?? 'unknown error'}`);
        ...
        defineTool({
            ...
            async execute(args) {
                ...
                if (gateFrom !== undefined)
                    return { ok: false, error: gateFrom };
                const gateTo = gatePath(check.state, args.to);
                if (gateTo !== undefined)
                    return { ok: false, error: gateTo };
                return run(async () => {
        ...
        defineTool({
            ...
            output: {
                ...
                render: (_args, value) => {
                    if (value.ok !== true)
                        return text(`remote_glob failed: ${value.error ?? 'unknown error'}`);
            ...
            async execute(args) {
                ...
                return run(async () => {
                    ...
                    if (root === undefined)
                        return { ok: false, error: 'remote root is not set' };
        ...
        defineTool({
            ...
            output: {
                ...
                render: (_args, value) => {
                    if (value.ok !== true)
                        return text(`remote_grep failed: ${value.error ?? 'unknown error'}`);
