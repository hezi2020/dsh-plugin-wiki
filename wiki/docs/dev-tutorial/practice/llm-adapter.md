...
async function* exampleChunks(): AsyncIterable<StreamChunk> {
  // 1. 用 block-start 开始每个内容块
  yield { type: 'block-start', index: 0, blockType: 'text' }

  // 2. 通过 text-delta 流式输出文本
  yield { type: 'text-delta', index: 0, text: 'Hello' }
  yield { type: 'text-delta', index: 0, text: ' world' }

  // 3. 用 block-end 结束每个内容块，并附带完整 block
  yield {
    type: 'block-end',
    index: 0,
...
请覆写 `resolveModel(provider, model, signal?)`，在一次查询中返回确切的提供方／模型身份以及可选的 `context` 和 `reasoning` 元数据。异步查询必须响应该可选信号，使取消和资源释放过程完全停稳。
