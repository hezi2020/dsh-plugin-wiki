...
!!! warning "不要导出普通对象作为 Config"
    插件加载时，Cordis 会通过导出的 schema 校验配置，并填充未提供字段的默认值。普通对象不满足 Cordis 要求的 Standard Schema 接口。
...
在 schema 中表达自身完备的约束，使无效配置在插件加载时失败。对服务或已注册资源的引用需要依赖注入；[服务教程](../framework/service.md)介绍了这项约定。
...
- [插件与生命周期](../framework/lifecycle.md) — 深入了解插件的完整生命周期
