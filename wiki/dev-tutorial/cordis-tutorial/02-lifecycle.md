...
export function apply(ctx: Context) {
  // 挂载一个子插件并保留其 fiber 以便之后释放
  const fiber = ctx.plugin(heartbeat)
  // 这个 demo 定时器本身也是 effect：如果本插件先被卸载，
  // 待执行的回调会被取消，而不是在一个已死亡的应用上触发
  ctx.effect(() => {
...
你会在[第 6 章](./06-composition-hmr.md)再次遇到 PENDING，它通常就是「为什么我的插件没有输出」的答案。
...
- 服务注册属于 effect。`ctx.tools.register(...)` 等 harness 注册表也会把返回的 disposer 附着到调用插件上，因此会自动撤销（[第 7 章](./07-into-harness.md)）。

!!! note "异步 disposer 的顺序"
