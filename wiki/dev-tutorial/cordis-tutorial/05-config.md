...
导出的 `Config` 既是 TypeScript 接口，也是同名的运行时 schema：消费方获得类型，Cordis 获得验证器。

!!! warning "使用 Schemastery，不是普通对象"
    本仓库使用 [Schemastery](https://github.com/shigma/schemastery) 定义 schema；Cordis 本身接受任意 [Standard Schema](https://standardschema.dev/) 验证器，因此将普通对象导出为 `Config` 无法工作。
...
!!! info "通过 schema 但引用不可用也要拒绝"
    如果某个插件的配置通过了 schema 验证，但其中指定的资源或提供方不可用，该插件也应当在能解析该引用时立即拒绝。
...
下一章：[组合与 HMR（热模块替换）](./06-composition-hmr.md)：将 `cordis.yml` 视为应用。
